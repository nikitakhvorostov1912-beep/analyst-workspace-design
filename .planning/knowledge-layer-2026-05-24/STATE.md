...
>
> **▶ НАПРАВЛЕНИЕ с 2026-06-02 (решение Никиты):** крупные камни M-K3 (EPF/CFE,
> BSL LS detector) — **ОТЛОЖЕНЫ** (EPF/CFE = вне этого репо, 1С-extension/EDT;
> BSL LS detector = не блокер релиза, требует Java на машине). Переходим в режим
> **bug-fixing + РЕДИЗАЙН приложения по ТЗ** (ТЗ от пользователя — ОЖИДАЕТСЯ).
> Релизный гейт (golden-set G1 + слепой тест G3) — позже, нужен пользователь.
>
