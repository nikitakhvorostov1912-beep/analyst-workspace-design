...
> - **DB durability ✅** — `up.ps1` теперь ставит `DATABASE_URL → pilot.db` на каждый `/awd-dev-up` (раньше backend читал пустую `/data/app.db`). 4 конфига / 63 207 карточек отдаются. (`.env` мне недоступен по правам — durability через launch-скрипт.)
>
