...
> - **Carryover редизайна — ДОБИТО (2026-06-03):** ✅ «Повторить ответ» (re-ask, проводка page→Thread→Message→AssistantMessage) · ✅ переименование чатов (inline-edit, backend PATCH уже был). **Остаток:** закрепление чатов (нужно поле `pinned` + миграция) · свёрнутый-режим-иконки · F-07 развилка ролей (skip-if-IT-configured) · F-05 полный аудит «декоративный оранжевый» · аудит доступности (DoD-чеклист).
...
>
> **🐞 ОТКРЫТЫЕ ХВОСТЫ (бэклог багов/долга — разбираем в bug-fix режиме):**
> - **🆕 `@`-mention не ищет объекты 1С (2026-06-03, репорт Никиты):** в композере
>   `@номе…` → попап «Объекты 1С» показывает «Ничего не найдено» (ожидается
>   Номенклатура и пр.). Где: `MentionPopover` + `lib/api.metadataSuggest` +
>   backend `/metadata/suggest` (route `metadata_suggest`). Вероятная причина:
>   metadata-кэш канала не проиндексирован (suggest читает кэш, а не живой MCP),
>   ИЛИ suggest не доходит до активного канала/пустой индекс. Проверить:
>   запущен ли indexer для канала Test, что отдаёт `/metadata/suggest?q=Номе`.
> - **ИТС медленный** — модель over-callает `buddy.search_its` (turn 1–3 мин на
>   методический вопрос): латентность живого Напарника + нет кэша/лимита повторов.
> - **Мёртвая «КА Демо» :6012** — подключение не отвечает, `/status` показывает
>   «1 из 2» красным. Убрать или починить (P2).
...
> - **Тех-долг (#14–17):** E501/F841 в `loop.py`, MetricCard→CardHeader (1/6 cards
>   refactor), 7 flaky-тестов (cp1251 на Windows), SMOKE-NIM-REBUILD.md.
