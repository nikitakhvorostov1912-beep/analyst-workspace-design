...
> - **Carryover редизайна:** F-11 закрепление/переименование чатов (нужен backend PATCH title + поле pinned) + свёрнутый-режим-иконки · F-07 развилка ролей (skip-if-IT-configured) · F-05 полный аудит «декоративный оранжевый» · «Повторить ответ» (проводка page→Thread) · аудит доступности (DoD-чеклист).
