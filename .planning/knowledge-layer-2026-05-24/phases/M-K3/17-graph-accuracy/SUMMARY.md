...
### ★ 0.5.0 — ОБНОВЛЕНИЕ 2026-06-02 (перекрывает B/C ниже — блокеры сняты)

> Самое свежее. Пункты B («граф устарел», «pilot.db залочен NIM») и C (план
> пересборки) — ВЫПОЛНЕНЫ, больше не актуальны.

- **`pilot.db` пересобран (M-K2.5, 4 конфы) — граф НЕ устарел, NIM завершён, БД НЕ залочена.** Блокер из B снят.
- **Грунтинг в чате РАБОТАЕТ end-to-end** (live MiMo, база Test :6010, Напарник :6002):
  - **Граф:** чат зовёт `trace_typical_movements` → 44 документа из графа.
  - **Карточки:** `explain_typical_object` → полная карточка (реквизиты/движения/связи/сценарии).
  - **ИТС:** `buddy.search_its` → `buddy.fetch_its` → ответ с живыми ссылками its.1c.ru.
- **#38 (цепочка молчит до лимита) ЗАКРЫТ** — grounding отвечает, не зацикливается.
- **Корень ИТС-бага (8cbd96c):** чат собирал tools через `MCPPool` (toolkit + только bsl-context), а buddy жил в неиспользуемом чатом `MCPOrchestrator` → `buddy.search_its` не доходил до LLM. Фикс: `BuddyAuxClient` (HTTP-aux, префикс `buddy.` + телеметрия) в `mcp_pool.build_aux_clients`. **Промпт-гайд `7ce7727` был корректен — дыра была в проводке, не в промпте.**
- **Durability (8cbd96c):** `awd-dev-up/up.ps1` ставит `DATABASE_URL → pilot.db` на каждый запуск (раньше backend читал пустую `/data/app.db`). `.env` мне закрыт по правам — durability через launch-скрипт.
- **User-facing слой «3 источника» (4836300):** бейдж «●●● Источники» (live-статус база/типовая/ИТС) + секция «Источники знаний» на `/status` + чипы пустого экрана по источникам + 2 раздела гайда (Типовые, ИТС·Напарник) + блок в онбординге + **ярлык-источник под ответом (`AnswerSources` = гейт G0.2 «метка источника»)**. `next build` ✅, 11 unit-тестов.
- **Остаётся для приёмки:** golden-set ≥30 вопросов из практики (G1, нужен пользователь) · слепой тест аналитиков (G3) · отключить graph-card emit в loop.py. Опц.: ускорить ИТС (модель over-callает `buddy.search_its` → turn 1–3 мин) · OPS-2 телеметрия (#40.3).
