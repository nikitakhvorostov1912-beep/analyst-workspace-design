...
## FINDING-14 (P0) — NVIDIA NIM env-ключ 401 ✅ RESOLVED (2026-05-24)

**Статус:** ✅ RESOLVED. Подтверждено пользователем 2026-05-24 вечером: embedded NVIDIA NIM ключ работает корректно после ротации (или ключ не был отозван, а 401 был транзиентным).
**Verify в smoke v1.4.5:** проверка отправки сообщения «Расскажи про базу» через NVIDIA NIM endpoint должна вернуть SSE ответ без `llm_invalid_key`. Если 401 вернётся — повторить процедуру rotate ключа из секции Fix ниже.

---

### Историческое описание (до закрытия):
