...
export const metadata: Metadata = {
  ...
  description: "Полное руководство по работе с приложением: подключения, обработка 1С, сценарии, диагностика.",
};
...
const SECTIONS: GuideSection[] = [
    ...
    content: (
      <>
        ...
        <Callout variant="info">
          <strong>Три источника знаний.</strong> Ответы собираются из трёх мест,
          и модель сама выбирает нужное под ваш вопрос:
          <ul className="mt-2 space-y-1 pl-4 list-disc">
            <li>
              <strong>Ваша база</strong> — конкретные данные, структура, журнал
              регистрации (живая 1С через MCP).
            </li>
            <li>
              <strong>Типовая конфигурация</strong> — как устроена УТ / ERP / КА /
              БП: движения, цепочки вызовов, реквизиты (наш граф + карточки).
              Раздел{" "}
              <a href="#typical" className="text-[var(--accent)] hover:underline">
                «Типовые конфигурации»
              </a>
              .
            </li>
            <li>
              <strong>ИТС · Напарник</strong> — методики, стандарты, инструкции 1С
              (живая документация ИТС). Раздел{" "}
              <a href="#its" className="text-[var(--accent)] hover:underline">
                «ИТС и Напарник»
              </a>
              .
            </li>
          </ul>
          Статус каждого источника виден в шапке (бейдж «Источники») и на странице{" "}
        ...
        <ol className="space-y-3 pl-5 list-decimal text-[14px] text-[var(--fg-2)] leading-[1.6] mb-5">
          ...
          <li>
            ...
            — это NVIDIA NIM, один ключ на весь каталог моделей (Nemotron, DeepSeek,
            Qwen3-Coder, Llama, Mistral). Конкретная активная модель показана в шапке
            (бейдж справа) и переключается в Настройках → Модель ИИ. Свой ключ нужен
            только если хотите Cloud.ru (РФ-ДЦ, 152-ФЗ) или другой провайдер.
          </li>
          <li>
            <strong className="text-[var(--fg-1)]">Выберите базу в шапке</strong> —
            если подключение одно, оно подставится автоматически.
          </li>
          <li>
  ...
  // ----------------------------------------------------------------------
  // 3. Инструменты в 1С (что умеет помощник)
  // ----------------------------------------------------------------------
  {
    id: "mcp-tools",
    title: "Инструменты в 1С",
    content: (
      <>
        <Lead>
          Под капотом приложение выполняет 10 операций над базой 1С и
          дополнительно использует справочник по платформе. Вам их вызывать вручную не нужно
  ...
  // ----------------------------------------------------------------------
  // 3b. Типовые конфигурации (граф + карточки)
  // ----------------------------------------------------------------------
  {
    id: "typical",
    ...
    content: (
      <>
        <Lead>
          Это <strong>отдельный источник</strong> — не ваша база, а знание о том,
          как устроены сами типовые конфигурации 1С: УТ 11.5, ERP 2.5, КА 2.5,
          БП 3.0. Внутри — граф связей (что куда пишет движения, кто кого
          вызывает, какие права) и карточки на каждый объект (назначение,
          реквизиты, сценарии). Спрашивайте, когда вопрос про устройство
          типовой логики, а не про данные конкретной клиентской базы.
        </Lead>

        <Callout variant="info">
          В шапке есть селектор <strong>«Типовая»</strong> — выберите конфигурацию
          (например, УТ 11.5), чтобы вопросы шли по ней. Если не выбрали — модель
          сама уточнит или определит по тексту вопроса.
        </Callout>

        <ToolCard
          name="explain_typical_object"
          icon={<Boxes className="h-4 w-4" />}
          purpose="Карточка типового объекта: назначение, ключевые реквизиты, движения по регистрам, типовые сценарии, связанные объекты."
          whenToUse="«как устроен документ Реализация в УТ», «что за регистр ТоварыНаСкладах», «реквизиты Заказа клиента»"
          example="Как устроен типовой документ Реализация товаров и услуг в УТ 11.5"
          cardType="Object (типовой)"
        />
        <ToolCard
          name="trace_typical_movements"
          icon={<GitBranch className="h-4 w-4" />}
          purpose="Точный список регистров, по которым документ делает движения, из графа (не из прозы карточки). Числа и имена — факт."
          whenToUse="«куда пишет движения Приходный кассовый ордер», «по каким регистрам проводится Реализация»"
          example="По каким регистрам делает движения Приходный кассовый ордер в БП 3.0"
          cardType="Table / текст"
        />
        <ToolCard
          name="trace_typical_calls"
          icon={<Network className="h-4 w-4" />}
          purpose="Цепочка вызовов процедур/функций: кто кого вызывает (out) и кто вызывает данный метод (in). Для анализа влияния доработок."
          whenToUse="«кто вызывает РасчётСебестоимости», «что вызывает ОбработкаПроведения документа»"
          example="Кто вызывает процедуру проведения Реализации в УТ 11.5"
          cardType="Table / текст"
        />
        <ToolCard
          name="compare_with_typical"
          icon={<Search className="h-4 w-4" />}
          purpose="Сравнение объекта вашей базы с типовым: что добавлено/изменено относительно эталона. Полезно для оценки доработок."
          whenToUse="«чем наш документ Реализация отличается от типового», «какие реквизиты дописали»"
          example="Сравни наш документ Реализация с типовым УТ 11.5"
          cardType="Table"
        />

        <Callout variant="tip">
          <strong>Откуда данные:</strong> карточки пересобраны реальной LLM
          (без заглушек), граф построен из выгрузок типовых. Точные числа
          (сколько регистров, какие движения) модель берёт из графа, а не из
          текста карточки — поэтому им можно доверять.
        </Callout>
      </>
    ),
  },
  ...
  // ----------------------------------------------------------------------
  // 3c. ИТС и Напарник (живая методология)
  // ----------------------------------------------------------------------
  {
    id: "its",
    title: "ИТС и Напарник",
    content: (
      <>
        <Lead>
          Третий источник — <strong>живая документация ИТС</strong> через сервис
          1С:Напарник. Сюда модель идёт за методиками, стандартами и инструкциями:
          «как правильно сделать X по 1С». Информация на ИТС меняется постоянно,
          поэтому это приоритетный источник для методических вопросов — модель
          цитирует конкретные статьи со ссылками на its.1c.ru.
        </Lead>

        <ToolCard
          name="buddy.search_its"
          icon={<BookMarked className="h-4 w-4" />}
          purpose="Поиск по живой базе ИТС — возвращает релевантные статьи (методики, инструкции, описания механизмов конфигураций)."
          whenToUse="«как закрыть месяц в УТ», «методика распределения расходов», «как настроить интеркампани» — вопросы про то, КАК правильно, а не про данные базы"
          example="Как по методике ИТС правильно закрыть месяц в УТ 11.5? Дай ссылки на статьи"
          cardType="текст + ссылки ИТС"
        />
        <ToolCard
          name="buddy.fetch_its"
          icon={<FileText className="h-4 w-4" />}
          purpose="Полный текст конкретной статьи ИТС по её идентификатору (после search_its). Модель использует автоматически, чтобы процитировать детали."
          whenToUse="Автоматически — следом за buddy.search_its, когда нужен полный текст найденной статьи."
          cardType="текст + ссылка"
        />

        <Callout variant="warning">
          <strong>Методические вопросы отвечаются медленнее.</strong> Живой
          Напарник ищет 20–50 секунд на каждый запрос, а модель обычно делает
          несколько уточняющих поисков — поэтому ответ на сложный методический
          вопрос может занять 1–3 минуты. Это нормально: вы получаете актуальную
          ИТС-информацию со ссылками, а не выдумку модели. Вопросы по вашей базе и
          по типовой (без ИТС) отвечаются быстро.
        </Callout>

        <Callout variant="info">
          Если Напарник недоступен (не запущен сервис на машине), приложение
          честно об этом скажет, а статус «ИТС» в шапке станет красным. При
          наличии локального индекса ИТС он используется как запасной источник.
        </Callout>
      </>
    ),
  },
      ...
      <>
        ...
        <ul className="space-y-2 pl-5 list-disc text-[13.5px] text-[var(--fg-2)] leading-[1.6] mb-4">
          ...
          <li>
            ...
            <Code>qwen/qwen3-coder-plus</Code>). По умолчанию идёт модель через
            NVIDIA NIM (вшит ключ); активная всегда видна в бейдже шапки.
          </li>
        </ul>
        <Callout variant="info">
          Базовый провайдер — NVIDIA NIM с вшитым ключом (один ключ на каталог:
          Nemotron, DeepSeek, Qwen3-Coder, Llama, Mistral, GLM и др.). Для 152-ФЗ —
          Cloud.ru Foundation Models (РФ-ДЦ). Можно подключить любого провайдера
          c OpenAI-совместимым API (включая Xiaomi MiMo) через «Свой endpoint».
        </Callout>
    ...
    content: (
      <>
        ...
        <ul className="my-3 space-y-1.5 pl-5 list-disc text-[13.5px] text-[var(--fg-2)] leading-[1.6]">
          ...
          <li>Сколько инструментов отвечает (обычно 10 операций)</li>
        ...
        <div className="my-5 space-y-3">
          <div className="rounded-md border border-[var(--bd-2)] bg-[var(--bg-1)] p-3.5">
            ...
            <ul className="space-y-1 pl-5 list-disc text-[13px] text-[var(--fg-2)]">
              <li>Запущена ли обработка-обработчик в самой 1С? (Сервис → Внешние обработки → имя файла <Code>MCP_Toolkit_v1.7.0.epf</Code>)</li>
